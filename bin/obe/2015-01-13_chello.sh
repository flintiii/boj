...
for prog in curl expect cadaver # debug banana
do
# echo $prog
isit=$(which $prog)
# echo $isit 
...
echo "This is the \""$FUNCNAME"\" function in "$0" version "$version #debug
showt > /dev/null
echo $var2 > site.title
curl -T site.title http://$uid:$pass@$tiddleurl/$proj/tiddlers/SiteTitle
...
# curl -T banana.title http://the:end@docbox.flint.com:1980/banana/tiddlers/SiteTitle
showt > /dev/null
echo $var2 > site.stitle
curl -T site.stitle http://$uid:$pass@$tiddleurl/$proj/tiddlers/SiteSubtitle
rc=$?; if [[ $rc != 0 ]]; then echo "error"; exit $rc; fi
echo $rc; 
spause
} # Test:
#
#
#* function mktdl - populates the grant proposal 
function mktdl(){
#debug 
showt > /dev/null
echo "This is the \""$FUNCNAME"\" function in "$0" version "$version #debug
# echo $tiddle
echo $tiddle > site.$tiddle
curl -T site.$tiddle http://$uid:$pass@$tiddleurl/$proj/tiddlers/$tiddle
...
if [ "$(id -u)" != "0" ]; then
   echo "This script does not need to run as root but that's OK you are not" 1>&2
   exit 1
fi
